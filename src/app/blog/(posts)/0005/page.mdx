...
>
> The section where I talk about bundle sizes and profiling page loading is not going to reflect the latest versions of the mentioned frameworks and should be taken with healthy skepticism. But you should be doing that with most information on the internet anyway right?
...
I was skeptical about migrating to [Next.js](https://nextjs.org) initially as I thought it was fairly heavy-handed like Angular would be but with React flavoring. But the option to be flexible between [server-side rendering (SSR)](https://nextjs.org/docs/pages/building-your-application/rendering/server-side-rendering) or [static site generation (SSG)](https://nextjs.org/docs/pages/building-your-application/rendering/static-site-generation) was really appealing for my use case. Plus there were ways to improve blog writing by using [MDX] plugins; this system was a pain to configure until I realized I was missing [Tailwind Typography](https://github.com/tailwindlabs/tailwindcss-typography) to merge with the Tailwind styling I've been using prior.
...
For this analysis, I will ignore the pure HTML and Angular versions of the site since they have other problems that make it an unequal comparison (for example, the Angular version has no code-splitting). Using the SSR/SSG vs CSR explanation from the previous section, here are a few areas that the migration from pure React to Next.js showed improvements in:
...
The React version of this site is not much better. Because I used [Vite](https://vite.dev/) instead of [webpack](https://webpack.js.org/) the results look different but provide the same information.
...
Let's look at how a webpage is loaded and rendered (in a semi-reductive manner). I'm skipping things like [DNS Resolution](https://www.cloudflare.com/learning/dns/what-is-dns/) and [TLS](https://en.wikipedia.org/wiki/Transport_Layer_Security); The point I'm trying to illustrate is more about how the client (a.k.a. the browser) processes information. [MDN has a great article explaining everything in greater detail ](https://developer.mozilla.org/en-US/docs/Web/Performance/How_browsers_work) and is great documentation to read when learning how to make faster websites. Overall, the process is broken down into a few steps:

1. The HTML is sent from the server to the client
2. The client **parses** the HTML
   1. If there is a `<link>` or `<script>` tag, a request is sent to fetch the resource. [This is usually blocking in the parsing of a webpage](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/script#notes)
   2. Assets like `<img>` or `<video>` tags are usually requested asynchronously; These requests do not block parsing but does block rendering.
3. Using the parsed data and fetched resources, the client begins **rendering** the HTML on the screen. This includes calculating styles, element sizes, etc.
4. After rendering, JavaScript is executed on the rendered page.




Angular is a very heavy-handed solution for something as simple as a static portfolio site: no dynamic data sources, no API requests, just a bit of JS for routing and interactivity. React at its core is small, but the reliance on [JSX](https://react.dev/learn/writing-markup-with-jsx) usually causes the UI to load after the minified JS is fetched by the browser. Even with lazy-loading components and routes there is still the notion of a "critical path" or sequence of requests that must occur for the page to load and then render.

First things first, bundle sizes should be looked at to see "how heavy" the website is to download and then potentially load.  (excludes resources like images)

Here is the Angular bundle,

```
Initial Chunk Files           | Names         |  Raw Size | Estimated Transfer Size
main.5d615bb339b9470b.js      | main          | 631.47 kB |               141.73 kB
styles.3792df0daddea44e.css   | styles        | 235.83 kB |                30.05 kB
polyfills.f902bb51b2ac1766.js | polyfills     |  33.07 kB |                10.65 kB
runtime.332e9e9b7bbd6596.js   | runtime       |   1.04 kB |               599 bytes

                              | Initial Total | 901.42 kB |               183.02 kB

Warning: bundle initial exceeded maximum budget. Budget 500.00 kB was not met by 401.42 kB with a total of 901.42 kB.
```


This is a lot of data for a very lightweight site. Sure, it contains all the data for every page on the site and all of JS needed to supply the interactivity but the metrics and the warning indicate otherwise. The first interesting metric is the difference between the _Raw Size_ and _Estimated Transfer Size_. While modern servers and browsers have [compression for HTTP requests/responses](https://developer.mozilla.org/en-US/docs/Web/HTTP/Compression) with nominal overhead, the disparity in the reduced transfer size indicates that _there is a lot of duplicate code and data being transferred_. It also implies that there could be a lot less JavaScript being run on the clients but the [transpilation](https://stackoverflow.com/questions/48414941/what-is-transpiler-and-what-is-exactly-work-of-transpiler) process couldn't optimize it that well.