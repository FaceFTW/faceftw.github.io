...
import googleSearch from './goggle-search.png';
...
import charmap from './charmap.png';
...
For the rewrite, I used [`winnow`](link) which is actually a fork of `nom`, but with API differences and built-in debugging features. In fact, `winnow`'s debugging is really cool since all that is needed is passing a Cargo feature flag:
...
One significant difference between `nom` and `winnow` is how to iterate over the input. In the original implemenation, `nom` has a predefined system for having an iterative parser, which I composed a `scan()` iterator over to track some nuances with when to interpret new lines. For better or worse, `winnow` doesn't really have that kind of default implementation (that I could fine), but the pieces to create a custom iterator (i.e. one that implements Rusts's [`Iterator` trait](rustdoc)) are there. Given the extra state management that was already required (that I will not detail but the code is there), this makes more sense to allow for a consolidated interface to interact with the parser,
To create an implementation of `Iterator`, the only required function to implement is `next()`. This returns an `Option<Self::Item>` using the associated type defined with the `Iterator` trait. The fun catch is because the parsers now return `Vec<TerminalCharacter>`, the iterator should actually have the associated type of `TerminalCharacter` and act as if [`flatten()`](rustdoc) was applied to the iterator. And this gets really interesting when realizing that struct members can't define _trait bounds as their type without using dynamic dispatch (the `dyn` keyword, which requires heap usage)_. This is a _worse_ solution since it means `Box`ing things that in reality don't need it. A "hack" is to use the structs defined in `std::iter` that are returned when composing iterators since it would allow for the type system to identify the correct implementation of `next()` to use, but there is a problem...

```rust
// Rust Playground Link:
// https://play.rust-lang.org/?version=stable&mode=debug&edition=2024&gist=643450d365c439970b0d4c6674db6e3d

struct ParserIter<'a, P> {
    inner: std::iter::Filter< std::slice::Iter<'a,&'a str>, P>,
}
...
where
    P: FnMut(&str) -> bool,
{
    pub fn new(inner: std::slice::Iter<'a, &'a str>) -> Self {
        Self {
            inner: inner.filter(|x| x != "1"),
        }
    }
}
...
fn main() {
    let slice = ["0", "1", "2", "3"].as_slice().into_iter();
...
The core issue here relates to  is that the iterator structs that have a predicate generic type like [`std::iter::Scan`](rustdoc) or [`std::iter::Filter`](rustdoc) are bounded by some variation of the [`FnMut`](rustdoc) trait.